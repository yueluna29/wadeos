...
export const TimeCapsulesView = () => {
  ...
  if (viewingCapsule && selectedCapsuleData) {
    ...
    return (
      <div className="h-full bg-[#f9f6f7] flex flex-col relative overflow-hidden">
        {/* Top Navigation */}
        <div className="absolute top-0 left-0 w-full p-6 z-20 flex justify-between items-start pointer-events-none">
          ...
          <div className="flex items-center gap-3 pointer-events-auto">
             {/* Listen Button */}
             <div className="flex items-center gap-2 bg-white/90 backdrop-blur-md rounded-full p-1 border border-[#eae2e8] shadow-sm">
                <button
                  onClick={() => handleListenClick(false)}
                  disabled={isLoadingAudio}
                  className={`w-8 h-8 rounded-full flex items-center justify-center transition-all
                    ${isPlayingAudio && !isPaused
                      ? 'bg-[#d58f99] text-white shadow-md' 
                      : 'bg-[#fff0f3] text-[#d58f99] hover:bg-[#d58f99] hover:text-white'
                    }
                ...
                {selectedCapsuleData.audioCache && (
                  <button
                    ...
                    disabled={isLoadingAudio}
                    className="w-8 h-8 rounded-full bg-transparent text-[#917c71] flex items-center justify-center hover:text-[#d58f99] transition-all"
                  >
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar pt-24 pb-12 px-4">
           <div className="max-w-md mx-auto bg-white rounded-[32px] shadow-sm border border-[#fff0f3] p-8 relative overflow-hidden min-h-[60vh]">
              {/* Decorative Elements */}
              <div className="absolute top-0 right-0 w-40 h-40 bg-gradient-to-bl from-[#fff0f3] to-transparent rounded-bl-[100px] -mr-10 -mt-10 opacity-60 pointer-events-none"></div>
              
              {/* Date Badge */}
              <div className="relative flex justify-center mb-8">
                 <span className="inline-flex items-center px-3 py-1 rounded-full bg-[#f9f6f7] border border-[#eae2e8]/60 text-[10px] font-bold text-[#917c71] uppercase tracking-wider">
                   {dateString}
                 </span>
              </div>

              {/* Title */}
              <h1 className="relative font-hand text-3xl text-[#5a4a42] mb-8 text-center leading-tight">
                {selectedCapsuleData.title || "A Letter from Wade"}
              </h1>

              {/* Divider */}
              <div className="flex justify-center mb-8">
                <div className="w-12 h-1 bg-[#d58f99]/20 rounded-full"></div>
              </div>
              
              {/* Content */}
              <div className="relative prose prose-pink max-w-none text-[#5a4a42]/90 leading-relaxed text-sm [&_p]:mb-4">
                <Markdown remarkPlugins={[remarkBreaks]}>{selectedCapsuleData.content}</Markdown>
              </div>

              {/* Footer Info */}
              <div className="mt-12 pt-6 border-t border-[#eae2e8]/60 flex justify-center">
                 <p className="text-[10px] font-bold text-[#d58f99]/40 uppercase tracking-[0.2em]">
  ...
  return (
    <div className="h-full bg-[#f9f6f7] flex flex-col overflow-hidden relative">
      <div className="flex-1 flex flex-col max-w-md mx-auto w-full p-4 pt-6 pb-4">
        ...
        <div className="bg-white rounded-[24px] shadow-sm border border-[#eae2e8]/60 mb-6 overflow-hidden flex-shrink-0">
          <div className="px-4 py-3 flex items-center justify-between border-b border-[#eae2e8]/40 bg-[#fff0f3]/30">
            <h2 className="text-base font-bold text-[#5a4a42]">
              {monthNames[currentDate.getMonth()]} <span className="text-[#d58f99]">{currentDate.getFullYear()}</span>
            </h2>
            <div className="flex gap-1">
              <button onClick={prevMonth} className="w-7 h-7 rounded-full hover:bg-[#fff0f3] flex items-center justify-center text-[#917c71] hover:text-[#d58f99] transition-colors">
                <Icons.ChevronLeft className="w-4 h-4" />
              </button>
              <button onClick={nextMonth} className="w-7 h-7 rounded-full hover:bg-[#fff0f3] flex items-center justify-center text-[#917c71] hover:text-[#d58f99] transition-colors">
          ...
          <div className="p-3">
            <div className="grid grid-cols-7 gap-y-1 text-center">
              {dayNames.map(day => (
                <div key={day} className="text-[9px] font-bold text-[#d58f99]/60 tracking-widest uppercase mb-1">{day}</div>
              ))}
                  ...
                  <div key={day} className="flex justify-center items-center h-8">
                    <button
                      ...
                      className={`w-7 h-7 rounded-full flex flex-col items-center justify-center text-xs font-bold transition-all relative
                        ${isSelected 
                          ? 'bg-[#d58f99] text-white shadow-sm scale-105' 
                          : isToday
                            ? 'bg-[#fff0f3] text-[#d58f99] border border-[#d58f99]/30'
                            : 'text-[#5a4a42] hover:bg-gray-50'
                        }
                      `}
                    >
                      <span>{day}</span>
                      {hasCapsule && !isSelected && (
                        <span className="absolute bottom-1 w-0.5 h-0.5 bg-[#d58f99] rounded-full"></span>
                      )}
                    </button>
                  </div>
                );
            ...
            <div className="flex items-center justify-between mb-4 px-2 flex-shrink-0">
              <div className="flex items-center gap-3">
                <div className="flex flex-col">
                  <span className="text-[10px] font-bold text-[#d58f99] tracking-widest uppercase mb-0.5">Selected Date</span>
                  <h3 className="font-bold text-[#5a4a42] text-xl font-serif">
                    {selectedDate.getDate()} {monthNames[selectedDate.getMonth()]}
                  </h3>
                </div>
              </div>
              <div className="bg-white px-3 py-1.5 rounded-xl shadow-sm border border-[#eae2e8] flex items-center gap-2">
                <span className="w-2 h-2 rounded-full bg-[#d58f99]"></span>
                <span className="text-xs font-bold text-[#917c71]">
              ...
              {selectedDayCapsules.length === 0 ? (
                ...
                >
                  <div className="w-14 h-14 bg-[#fff0f3] rounded-full flex items-center justify-center mb-3 group-hover:scale-110 transition-transform duration-300">
                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#d58f99" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                      <path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path>
                      <path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path>
                    </svg>
                  </div>
                  <h4 className="font-bold text-[#5a4a42] mb-1">Write a Memory</h4>
                      ...
                      return (
                        <div key={cap.id} className="min-w-full snap-center relative group perspective-1000 px-1">
                          <div 
                            ...
                            ${isAvailable 
                              ? 'bg-white shadow-[0_10px_40px_-10px_rgba(213,143,153,0.2)] cursor-pointer border border-[#fff0f3]' 
                              : 'bg-[#f8f6f6] border border-[#eae2e8] cursor-not-allowed'
                            }
                          `}>
                            {/* Decorative Background Elements for Unlocked */}
                            {isAvailable && (
                               <>
                                 <div className="absolute top-0 right-0 w-32 h-32 bg-gradient-to-bl from-[#fff0f3] via-[#fff5f7] to-transparent rounded-bl-[100px] -mr-8 -mt-8 opacity-60 pointer-events-none"></div>
                                 <div className="absolute bottom-0 left-0 w-20 h-20 bg-[#fff0f3] rounded-tr-[80px] -ml-6 -mb-6 opacity-40 pointer-events-none"></div>
                               </>
                            )}
                            ...
                            <div className="relative p-5 flex flex-col h-full">
                              <div className="flex items-start gap-4 mb-4">
                                ...
                                <div className={`
                                  ...
                                  ${isAvailable 
                                    ...
                                    : 'bg-white text-[#917c71] border border-[#eae2e8]'
                                  }
                                `}>
                                  {isAvailable ? '💌' : '🔒'}
                                </div>
                                ...
                                <div className="flex-1 min-w-0 pt-0.5">
                                  <div className="flex justify-between items-start mb-1.5">
                                     <h4 className={`font-bold text-base pr-2 leading-tight ${isAvailable ? 'text-[#5a4a42]' : 'text-[#917c71]'}`}>
                                       {cap.title || "A Letter from Wade"}
                                     </h4>
                                     <span className="text-[10px] font-bold font-mono text-[#917c71]/60 bg-[#f9f6f7] px-2 py-1 rounded-full border border-[#eae2e8]/50 whitespace-nowrap flex-shrink-0">
                                       {timeStr}
                                     </span>
                                  </div>
                                </div>
                              ...
                              <p className={`text-xs line-clamp-4 mb-auto leading-relaxed ${isAvailable ? 'text-[#917c71] opacity-90' : 'text-[#917c71]/50'}`}>
                                {isAvailable 
                              ...
                              {/* Footer / Action */}
                              <div className="flex items-center justify-between border-t border-[#eae2e8]/40 pt-4 mt-4">
                                 <div className={`text-[10px] font-bold tracking-wider uppercase flex items-center gap-1.5
                                   ${isAvailable ? 'text-[#d58f99]' : 'text-[#917c71]/60'}
                                 `}>
                                   {isAvailable ? (
                                     <>
                                       <span className="relative flex h-2 w-2">
                                         <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-[#d58f99] opacity-75"></span>
                                         <span className="relative inline-flex rounded-full h-2 w-2 bg-[#d58f99]"></span>
                                       </span>
                                       Available Now
                                     </>
                                   ) : (
                                 ...
                                 {isAvailable ? (
                                   <div className="text-xs font-bold text-[#d58f99] flex items-center gap-1 group-hover:gap-2 transition-all">
                                   ...
                                   <button
                                      ...
                                      className="w-7 h-7 rounded-full bg-white border border-[#eae2e8] text-[#917c71] flex items-center justify-center hover:text-[#d58f99] hover:border-[#d58f99] transition-colors"
                                   >
                  ...
                  {selectedDayCapsules.length > 1 && (
                    <div className="flex justify-center gap-2 mt-4 flex-shrink-0">
                      {selectedDayCapsules.map((_, idx) => (
                        <div 
                          key={idx} 
                          className={`h-1.5 rounded-full transition-all duration-300 ${idx === activeSlideIndex ? 'bg-[#d58f99] w-4' : 'bg-[#d58f99]/30 w-1.5'}`}
                        />
                      ))}
                    </div>
                  )}
              ...
              <div className="p-6 overflow-y-auto max-h-[calc(85vh-180px)] custom-scrollbar">
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      <label className="block text-[10px] font-bold text-[#917c71] mb-1.5 uppercase tracking-wider">Unlock Date</label>
                      <input
                        type="date"
                        value={newCapsule.unlockDate}
                        onChange={(e) => setNewCapsule({ ...newCapsule, unlockDate: e.target.value })}
                        className="w-full px-0.5 py-2 rounded-xl border border-[#eae2e8] bg-[#fdfbfb] text-[#5a4a42] focus:outline-none focus:border-[#d58f99] text-xs transition-colors"
                      />
                    </div>
                    <div>
                      <label className="block text-[10px] font-bold text-[#917c71] mb-1.5 uppercase tracking-wider">Time</label>
                      <input
                        type="time"
