...
const Icons = {
  ...
  MoreHorizontal: () => (<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="1"></circle><circle cx="19" cy="12" r="1"></circle><circle cx="5" cy="12" r="1"></circle></svg>),
  ...
  Plus: () => (<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><line x1="12" y1="5" x2="12" y2="19"></line><line x1="5" y1="12" x2="19" y2="12"></line></svg>),
...
export const SocialFeed: React.FC = () => {
  ...
  // 🔥 修复白屏大 Bug：这里只存 postId，不存该死的数组 index！
  ...
  const PostCaption = ({ content, authorName, hideAuthor, className }: { content: string, authorName: string, hideAuthor?: boolean, className?: string }) => {
    ...
    // 判断是否需要显示 show more：字数大于 200 或者换行超过 5 行
    const needsShowMore = content.length > 200 || content.split('\n').length > 5;

    return (
      <div className={`text-[15px] text-wade-text-main leading-snug ${hideAuthor ? '' : 'px-4 pb-2'} ${className || ''}`}>
        <div className={`relative ${!isExpanded && hideAuthor ? 'line-clamp-5' : ''}`}>
          ...
          {/* 🔥 修复 Show more：取消下划线，阻止冒泡，点击在原地展开 */}
          {!isExpanded && hideAuthor && needsShowMore && (
            <button onClick={(e) => { e.stopPropagation(); setIsExpanded(true); }} className="text-[#1d9bf0] text-[15px] absolute bottom-0 right-0 bg-wade-bg-base pl-2 pr-1 pt-0.5">
              Show more
            </button>
          )}
        </div>
  ...
  const ImageCarousel = ({ images }: { images: string[] }) => {
    ...
    return (
      <div className="relative w-full aspect-video bg-black flex items-center justify-center overflow-hidden group border border-wade-border">
    ...
    return (
      <div className="flex-1 bg-wade-bg-base flex flex-col font-sans relative">
        ...
        <div className="flex-shrink-0 bg-wade-bg-base/90 backdrop-blur-md border-b border-wade-border px-4 py-3 flex items-center justify-between sticky top-0 z-40">
          ...
          <div className="font-bold text-[18px] text-wade-text-main absolute left-1/2 -translate-x-1/2">Post</div>
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-4 max-w-xl mx-auto w-full">
            <div className="flex gap-3 mb-3 cursor-pointer" onClick={() => setViewingProfile(author === 'Wade' ? 'Wade' : 'Luna')}>
               <img src={author === 'Wade' ? settings.wadeAvatar : settings.lunaAvatar} className="w-12 h-12 rounded-full border border-wade-border hover:opacity-80 transition-opacity" />
               <div className="flex flex-col justify-center leading-tight">
                  <span className="font-bold text-[15px] text-wade-text-main hover:underline">{author === 'Wade' ? 'Wade Wilson' : 'Luna'}</span>
            ...
            {currentPost.images && currentPost.images.length > 0 && (
              <div className="mb-3 rounded-2xl overflow-hidden border border-wade-border">
                {currentPost.images.length === 1 ? <img src={currentPost.images[0]} className="w-full object-cover cursor-zoom-in" onClick={() => setZoomedImage({images: currentPost.images, index: 0})} /> : <ImageCarousel images={currentPost.images} />}
              </div>
            )}

            <div className="text-wade-text-muted text-[15px] border-b border-wade-border pb-4 mb-2">
               {/* 🔥 修复时间：用上了绝对精准的格式 */}
               {formatExactTime(currentPost.timestamp)} · <span className="font-bold text-wade-text-main">WadeOS App</span>
            </div>
          ...
          <div className="flex-1 overflow-y-auto pb-24 custom-scrollbar bg-wade-bg-base">
            <div className="max-w-xl mx-auto">
              {localPosts.length === 0 ? (
                ...
                return (
                  <div key={post.id} onClick={() => setViewingPostDetail(post.id)} className="bg-wade-bg-base border-b border-wade-border hover:bg-black/[0.03] transition-colors cursor-pointer px-4 pt-3 pb-2 flex gap-3 font-sans">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex justify-between items-start mb-0.5">
                        <div className="flex items-center gap-1 text-[15px] overflow-hidden whitespace-nowrap">
                          ...
                          {/* 🔥 修复列表时间 */}
                          <span className="text-wade-text-muted hover:underline">{formatExactTime(post.timestamp)}</span>
                        </div>
