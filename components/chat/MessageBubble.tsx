...
export const MessageBubble: React.FC<MessageBubbleProps> = ({
  ...
  // 还原截图里的时间格式：3月8日 13:22
  const formatDateTime = (ts: number) => {
    const d = new Date(ts);
    return `${d.getMonth() + 1}月${d.getDate()}日 ${d.toLocaleTimeString('en-US', { hour12: false, hour: '2-digit', minute: '2-digit' })}`;
  };
  const formatTimeOnly = (ts: number) => new Date(ts).toLocaleTimeString('en-US', { hour12: false, hour: '2-digit', minute: '2-digit' });
  ...
  if (msg.isRegenerating) {
    return (
      <div className={`flex w-full mb-6 gap-3 animate-pulse ${isLuna ? 'justify-end pr-4' : 'pl-2'}`}>
  ...
  if (!isLuna) {
    return (
      <div className="flex w-full mb-8 gap-3 group animate-fade-in pl-2 pr-4">
        {/* 左侧：头像霸占侧边栏位置 */}
  ...
  // --- 状态 4: Luna (还原截图排版) ---
  return (
    <div className="flex w-full mb-8 gap-3 group animate-fade-in justify-end pl-4 pr-2">
      {/* 左侧：文字和气泡框 */}
